...
export default function ProductsContent({
  ...
  initialColors = [],
  initialSizes = [],
  initialPriceRange = { min: 0, max: 100000 },
}) {
  ...
  const [priceRange] = useState(initialPriceRange);
  ...
  const fetchProducts = useCallback(async (category, materials, colors, sizes, priceMin, priceMax, sort, search, page) => {
    try {
      setLoading(true);
      const params = { page: page || 1, limit: ITEMS_PER_PAGE };
      ...
      // Support both paginated { products, pagination } and legacy array response
  ...
  useEffect(() => {
    ...
    // Skip client-side fetch on initial render if we have server data and no filters applied
    if (isInitialRender.current) {
      isInitialRender.current = false;
      const hasFilters = category || materialsParam || colorsParam || sizesParam || minPrice || maxPrice || (sort && sort !== 'featured') || search || (page && page !== '1');
  ...
  }, [searchParams, fetchProducts, priceRange.min, priceRange.max, initialProducts.length]);
  ...
  const totalProducts = pagination ? pagination.total : products.length;
        ...
        <div className="mb-10" ref={headerRef}>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
            ...
            <p className="text-[11px] text-[color:var(--color-body)] uppercase tracking-[0.15em] font-medium" style={{ fontFamily: "var(--font-dm-mono, monospace)" }}>
              {loading ? 'Loading...' : `${totalProducts} Items Found`}
            </p>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {!loading && totalPages > 1 && (
              <div className="mt-12 flex justify-center items-center gap-2">
                <button
                  ...
                  className="p-2 border border-[color:var(--color-border-light)] text-[color:var(--color-heading)] disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[color:var(--color-heading)] hover:text-white transition-colors"
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((item, i) =>
                    item === '...' ? (
                      <span key={`dots-${i}`} className="px-2 text-[color:var(--color-body)]">...</span>
                    ) : (
                      <button
                        key={item}
                        ...
                        className={`w-10 h-10 text-sm font-mono border transition-colors ${
                          currentPage === item
                            ? 'bg-[color:var(--color-heading)] text-white border-[color:var(--color-heading)]'
                            : 'border-[color:var(--color-border-light)] text-[color:var(--color-heading)] hover:bg-[color:var(--color-heading)] hover:text-white'
                        }`}
                      >
                        {item}
                      </button>
            ...
            {!loading && products.length > 0 && (
              <div className="mt-6 text-center">
                <span className="text-[11px] text-[color:var(--color-body)] uppercase tracking-[0.15em]" style={{ fontFamily: "var(--font-dm-mono, monospace)" }}>
                  {pagination
                    ? `Showing ${(currentPage - 1) * ITEMS_PER_PAGE + 1}–${Math.min(currentPage * ITEMS_PER_PAGE, pagination.total)} of ${pagination.total} products`
